...
- [ ] **Mempool visibility** — the gap Ogmios closes that offline evaluation cannot: between submit and confirmation we cannot distinguish "accepted, waiting for a block" from "silently rejected". *No longer blocked* — Ogmios now runs against the devnet and `ada status` reports it, so this is buildable rather than theoretical
