...
- [x] **`localnet snapshot` / `localnet rollback`** — mark a point in the chain's history and return to it, which is how a bug that only appears on a *specific* chain state gets reproduced without replaying every transaction that led to it. Needed yaci-devkit **0.11.0-beta1**, since 0.10.6 is the newest stable and predates the endpoint; an older devkit is told which version it needs rather than shown a 404. Rolling back stops and restarts the node, and discards everything after the snapshot, so it is gated behind `--yes`
...
- [x] **The chain's cost models reach the builder, not only the evaluator** — MeshJS's `fetchCostModels` is a stub that throws on both providers we use, so the builder silently falls back to mainnet defaults. That is not cosmetic: the **script integrity hash** a transaction carries is computed over the cost models, so the wrong ones produce a hash the ledger refuses with `ScriptIntegrityHashMismatch` whether or not the script would have passed. The counts vary by network and protocol version — a devnet from an older genesis reports 166/175/297, preprod reports 332/332/350 — so whatever the chain says about itself is what it checks against
...
- [x] **Ogmios consumed if present, never required** — `ADA_OGMIOS_URL`, or the devkit's own default port on a local chain, never guessed for a public one. `status` reports whether it is there so its absence is visible rather than inferred. Probed over JSON-RPC rather than `/health`, because a hosted Ogmios behind a web front end answers that path with an HTML page and a 200
- [x] **`--verify-budget`** — `simulate` asks a node for the same execution units. Verified on preprod against Koios's Ogmios: both said mem 25,811 and steps 8,459,957 for the same transaction, one from a Plutus VM in JavaScript and one from the implementation that judges it. A missing second opinion never fails an operation that already has a first
- [x] **Mempool visibility** — `ada tx status <hash>` reports on-chain, in-mempool or not-found. The middle state needed a node: an indexer cannot see a mempool, so "not found" used to cover both "still queued, wait" and "dropped, act". Mempool monitoring is a stateful mini-protocol whose snapshot belongs to the connection, so unlike `--verify-budget` this opens a websocket rather than posting. Absent Ogmios it says so instead of guessing. `--wait` polls to confirmation
- [ ] **File the three upstream reports** — all drafted in `ada-dev-cli-internal/upstream/`. The two MeshJS ones are the serious half: `fetchCostModels` throwing makes the builder compute the script integrity hash from the wrong models, so **every Plutus transaction is rejected** on any network whose parameters differ from its defaults; and `complete()` writes a *draft's* execution budget, so a validator reading the finished transaction is under-funded and the node aborts it mid-script while appearing to blame the contract. Filing is the author's call
...
- [x] **Datums and redeemers validated against the declared schema** — CIP-57 gives every argument a `dataType` and enumerates a sum type's constructors, so a wrong alternative, a wrong field count, a number where bytes belong, or plain text that should be hex is caught before a transaction is built — naming the field and listing the valid alternatives. `mn` has to guess at this boundary; we do not. Deliberately permissive where the blueprint is: an untyped `Data` field or an unresolvable `$ref` accepts anything, because inventing a rule the compiler never stated would reject valid input
...
- [x] **Reference inputs, validity intervals, required signers** — `--read-only` for CIP-31 inputs read without being spent, `--signer` for the other parties a validator checks for, and `--valid-from`/`--valid-until`/`--valid-for` for the window a deadline validator reads. Durations are measured from the **chain tip**, never the local clock: a machine seconds fast produces a transaction not yet valid and one seconds slow produces an expired one, and both fail looking like anything but a clock
...
- [x] **Verified on preprod** — lock, simulate and unlock through the commands against a public network, with a one-hour validity window the devnet's short horizon refuses. Execution units came out identical to the devnet's, which is the ledger's promise that a script's cost is reproducible
- [x] **The same suite runs against preprod** — `ADA_TEST_NETWORK=preprod ADA_TEST_WALLET=… ADA_TEST_WALLET_2=… npm run test:chain`. All nine cases pass on a public network. Waits poll for the condition each step needs rather than sleeping a guessed duration, since twenty-second blocks make a fixed sleep meaningless
- [x] **Cross-checked against `cardano-cli`** — `contract address --cross-check` has the Haskell reference implementation compute the same hash, and **refuses to report an address the two disagree about**. Three implementations now agree on the hello-world validator: ours via MeshJS, Aiken's own, and cardano-cli's
...
- [x] **`dev`** — watches `validators/` and `lib/`, runs `aiken check` then `build` on save, and reports the tests and hash. Its real job is the address: a validator's address is a hash of its compiled code, so **every edit moves it**, and the loop says so and reports what is stranded at the one you just left. Two bugs found by using it — `runAiken` throws on a failed compile, which killed the loop on the first syntax error, and `check` does not write plutus.json, so every address came from a stale blueprint
