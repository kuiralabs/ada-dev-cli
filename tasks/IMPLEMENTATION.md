...
- [x] **Cost models validated, not merely fetched** — MeshJS's `fetchCostModels` is a *stub that throws* on both **Yaci and Koios**, so it silently falls back to **mainnet** cost models. Koios is our default for every public network, so this is not a devnet quirk. Both chains serve them, but they disagree: Yaci keys by parameter name and matches the ledger, while Koios keys by numeric index and reports 332 for PlutusV1 where the ledger fixes it at 166. A wrong cost model does not fail loudly — it yields a wrong execution budget, so either an overpaid fee or a script that aborts and forfeits collateral. The counts are therefore checked against the ledger's, and an implausible answer falls back to the built-in models **with the reason printed** rather than silently
